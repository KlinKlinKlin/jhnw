...
public class SwitchInteraction {
        ...
        for (int num = line_n;num<return_information_array.length; num++){
            ...
            if (!matched.equals("null")){
                ...
                if (matched_true_false){
                    ...
                    if ((analysis_true_string.indexOf("endmark") ==-1) && (analysis_true_string.indexOf("nextcmd") == -1)){
                        ...
                        if (operation.equalsIgnoreCase("loop")&&num<return_information_array.length){
                            ...
                            if (ProblemScanLogic_returnstring.get(1).indexOf("noproblem_endmark")!=-1 && line_n != return_information_array.length){
                ...
                //匹配失败
                    ...
                    //如果包含endmark 说明存在问题或者 不存在问题 有结果
                    if ((analysis_false_string.indexOf("endmark") !=-1) || (analysis_false_string.indexOf("nextcmd") !=-1)){
                        insertvalueInformationService(user_String,false,problemScanLogic,current_Round_Extraction_String);
                        current_Round_Extraction_String = "";
                    ...
                    }else if ((analysis_false_string.indexOf("endmark") ==-1) && (analysis_false_string.indexOf("nextcmd") ==-1) && (analysis_false_string.indexOf("complete") ==-1)){
                        //如果 不包含endmark  nextcmd  complete 说明 没有结果 是具体ID
                        //下一条分析ID+"=:="+当前行数
    ...
    public void insertvalueInformationService(List<String> user_String,boolean boo,ProblemScanLogic problemScanLogic,String parameterString){
        //第一条分析ID : haveproblem_endmark  noproblem_endmark
        String getNextId = "";
        //问题索引
        String problemId ="";
        //解决问题命令ID
    ...
    public String analysis_true(String way,SshMethod connectMethod,TelnetSwitchMethod telnetSwitchMethod,ProblemScanLogic problemScanLogic,int num){
        ...
        if (!tNextId_string.equals("noproblem_endmark")
            ...
            //如果 下一条分析ID 包含  noproblem_endmark haveproblem_endmark nextcmd
    ...
    public String analysis_false(String way,SshMethod connectMethod,TelnetSwitchMethod telnetSwitchMethod,ProblemScanLogic problemScanLogic,int num){
        ...
        if (!fNextId_string.equals("noproblem_endmark")
                ...
                && !fNextId_string.equals("complete")
            ...
            switch ( fNextId_string ){
                //false 存在问题——endmark：确认存在问题，是否返回看命令id
                case "haveproblem_endmark":
                    ...
                    //false 不存在问题——endmark：确认不存在问题，返回同上
                case "noproblem_endmark":
                    return analysis_false = "noproblem_endmark";
                //false nextcmd：代表需要执行命令才能进一步分析，看命令id
                case "nextcmd":
                    //返回待分析nextcmd 命令id
                ...
                case "complete":
                    return analysis_false = "complete";
                case "":
            }
        }
